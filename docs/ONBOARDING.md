...
| `docs/` | 本交接文档 + README 用的资源：`icon.png`（256px 应用图标）、`screenshots/`（4 张运行效果截图：device-list / system-status / remote-desktop-kvm / audit-log） |
...
- **图片裂图**：`commander.htm` 引用外部图片的形式有三种——CSS `url(images-commander/x.png)`、JS 拼接 `'images-commander/icons200-'+icon+'-1.png'`、**无引号 HTML 属性** `<img src=authcsme.png>`（正则找引用时容易漏最后一种）。图片必须随 `app/` 打包。v0.2.2 曾漏掉整个 `images/` 目录（TLS 锁图标 `lock.gif`/`unlock.gif`）；排查用"全面扫描法"：python 正则提取全部 10 个 htm 的图片引用（src 带/不带引号 + `url()` + JS 拼接前缀），与 `app/` 现有文件求差集一次补齐。
- **`cert1.png` 上游疏漏**：上游代码引用 `images-commander/cert1.png`，但上游仓库（本地快照与 GitHub 均查证）只有 `certificate-1.png` → 在 `app/images-commander/` 放同内容别名文件解决，不改 `upstream/`。
...
- **文件写入工具不落盘**：AI 工具链的 Write/StrReplace 对**已存在的**文件（`README.md`、本文档）多次报成功但不落盘（README 曾以空文件提交进 git 数个提交才发现；v0.2.2 重写 README 与更新本文档时均复发）→ 关键文件写完必须 `wc -l` / `grep` 验证；标准绕行：Write 到 `/tmp/` 下**新文件**（新路径可靠）验证后 `cp` 覆盖目标，或 `python3 heredoc` 直接写。
...
## 9. 【v0.2.2】TLS 裂图修复 + 双语 README

- **裂图修复**：补 `app/images/`（lock/unlock.gif）+ `app/images-commander/cert1.png` 别名（上游疏漏），详见第 5 节；全面扫描复检 10 个 htm 图片引用零缺失。
